...
1. When the user says "checkout and push" or similar, use this sequence: 
   git_stash → git_fetch (for the target branch) → git_checkout → git_stashPop → ask the user to stage files (user_confirm or user_addAll) → git_commit → git_push
...
7. Keep responses concise — the user is in a terminal.`;
